...
<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=d124ec974705ed3fb5dc8df5bbd68978&libraries=services"></script>
...
$(function(){
	...
	var signupForm = $("#signupForm");	// 가입하기 버튼
	var imgFile = $("#imgFile");	// 가입하기 버튼
	
	var idCheckFlag = false;			// 중복확인  flag(진행안함)
	
   imgFile.on("change", function(event){
	   var file = event.target.files[0];	// Open파일로 선택한 이미지 배열
	...
	idCheckBtn.on("click", function(){
		...
		$.ajax({
			...
			success : function(res){
				...
				var err = $(".error")[0];	// error 클래스를 가지고 있는 element 중 id요소에 해당하는 Element
				if(res == "NOTEXIST") {		// 아이디 사용가능
					...
					idCheckFlag = false;	// 아이디 중복 체크 진행했지만, 사용 불가능함
				}
			}
		});
	});
	// 상세주소 입력 후 포커스 아웃될 때, 지도가 표시됨
	memAddress2.on("click", function(){
	...
	signupBtn.on("click", function(){
		...
		var memId = $("#memId").val();			// ID
		var memPw = $("#memPw").val();			// PW
		...
		// 개인정보 처리방침을 동의하게 되면  Y값이 넘어오므로, 동의 여부는 true로 설정한다.
		...
		// 개인정보 처리방침을 동의 했습니까?
		// 아이디 중복체크를 이행하고 오셨나요?
		// 하고 오셧다고요? 그럼 가입하기를 진행할게요
		// 예? 안했어요? 그럼 가입하기 어렵지.. 안해줌 ㄹㅇ;;
		if(agreeFlag) {
			if(idCheckFlag) {
				signupForm.submit();
			}else {
