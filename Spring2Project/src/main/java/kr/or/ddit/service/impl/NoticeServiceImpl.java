...
public class NoticeServiceImpl implements INoticeService {
	...
	public ServiceResult idCheck(String memId) {
		...
		if(member != null) {
			...
			result = ServiceResult.NOTEXIST;	// 아이디와 일치하지 않는 회원정보
		}
	...
	public ServiceResult signup(HttpServletRequest req, NoticeMemberVO memberVO) {
		...
		// 넘겨받은 memberVO안에 들어있는 프로필 이미지 파일로 파일 업로드를 먼저 진행
		...
		try {
			...
			if(profileImgFile != null && profileImgFile.getOriginalFilename() != null &&
				...
				// 업로드 하기 위한 경로 생성(/resources/profile/UUID_원본파일명)
				uploadPath += "/" + fileName;
		...
		if(status > 0) {	// 등록 성공
			// 한명의 회원이 등록될 때 하나의 권한을 무조건 가질 수 있도록 권한 등록(스프링 시큐리티 적용 시 사용 예정)
	...
	private void noticeFileUpload(List<NoticeFileVO> noticeFileList, int boNo, HttpServletRequest req) {
		...
		if(noticeFileList != null && noticeFileList.size() > 0) {	// 넘겨받은 파일 데이터가 존재할 때
			...
			for(NoticeFileVO noticeFileVO : noticeFileList) {
				...
				// '/resources/notice/number/'와 같은 폴더 구조를 생성
	...
	private void deleteFolder(HttpServletRequest req, String path) {
		...
		try {
			if(folder.exists()) {	// 경로가 존재한다면
				File[] fileList = folder.listFiles();	// 폴더안에 있는 파일들의 목록을 가져온다.
				
				for(int i =0; i < fileList.length; i++) {
					if(fileList[i].isFile()) {	// 폴더안에 있는 파일이 파일일때 
						fileList[i].delete();
	...
	public NoticeFileVO noticeDownload(int fileNo) {
		...
		// 다운로드 횟수 증가
		noticeMapper.incrementNoticeFileDownCount();
