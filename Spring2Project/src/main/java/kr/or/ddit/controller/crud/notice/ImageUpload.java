...
public class ImageUpload {
	...
	public String imageUpload(
		...
		// 파일 객체가 null이 아니고 파일 사이즈가 0보타 크고 파일명이 공백이 아닌경우는 무조건 파일 데이터가 존재하는경우
		if(file != null && file.getSize() > 0 && StringUtils.isNotBlank(file.getOriginalFilename())) {
			...
			if(file.getContentType().toLowerCase().startsWith("image/")) {
				...
				if(file.getSize() > limitSize) {	// 크기 초과 에러
					 ...
					 * {
					 * 		"uploaded" : 1,
					 * 		"fileName" : "xxxxxxxxxxx-xxxxxxxxxxx.jpg",
					 * 		"url" : "/resources/img/xxxxxxxxxx-xxxxxxxxxx.jpg"
					 * }
					 */
					try {
