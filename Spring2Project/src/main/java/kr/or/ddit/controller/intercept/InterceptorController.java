...
public class InterceptorController {
	...
	/*
	 	...
	 	1. 인터셉터 셜명
	 		...
	 		하지만, 필터와 AOP를 활용하여 해당 기능의 needs를 달성할 수도 있습니다. 그렇지만, 제약도 많고 가용하기 위한 여러 파라미터 정보들의 종류가
	 		다릅니다. 그렇기 때문에, 필터와 AOP를 활용하는것보다 현재의 상황이라면 Interceptor를 활용하는게 훨씬 적합할 것입니다.
	 		...
	 		# 필터와 인터셉터
	 			...
	 			스프링 내의 객체에는 접근이 불가능합니다.
	 			
	 		
	 		# 스프링 AOP와 인터셉터
	 		
	 			특정 객체 동작의 시전 혹은 사호 처리는 AOP 기능을 활용할 수 있지만 컨트롤러의 처리는 인터셉터를 활용하는 경우가 더 많습니다.
	 			AOP의 어드바이스와 인터셉터의 가장 큰 차이는 파라미터의 차이라고 할 수 있습니다.
	 			어드바이스의 경우 JoinPoint와 ProceedingJoinPoint 등을 활용해서 호출 대상이 되는 메소드의 파라미터 등을 처리하는 방식입니다.
	 			인터셉터는 필터와 유사하게 HttpServletRequest, HttpServletResponse 파라미터로 받는 구조이다.
	 	...
	 	2. 인터셉터 구현
	 		...
	 		클라이언트의 요청을 처리하다 보면 요청 경로마다 접근 제어를 다르케 하거나, 특정  URl에 대한 접근 내역을 기록하고 싶을 대가 있습니다.
	 		...
	 		지금까지 배운 내용들을 떠올려보면 이를 해결할 수 있는 방식 2가지가 존재한다는걸 알 수 있습니다. (필터와 AOP)
	 		
	 		그 첫 번째로 Filter라는 서블릿 스펙에 따른 객체를 사용하는 방법이 있습니다. 필터를 통해 DispatcherServlet이나 컨트롤러에게 요청이
	 		위임되기 전/후에 공통적인 어떤 기능을 수행하도록 하면, 기존 컨트롤러나 DispatcherServlet에 어떤 수정을 가하거나 코드 중복없이
	 		...
	 		점입니다. 물론 아예 불가능한건 아닙니다. 설정이나 다른 부분이 복잡하고 귀찮을뿐...
	 		...
	 		몇가지 불편한 점들이 있습니다. 반드시 위임을 받을 빈은 Filter를 구현하고 있어야하고, root-context에서 관리되는 빈이어야 한다는 점입니다.
	 		
	 		두 번째로 AOP방법론에 따라 공통 기능을 정의한 Advice를 구현하고, pointcut을 통해 적절한 target 컨트롤러를 골라낸 다음 두 설정으로
	 		Aspect를 생성하여 런타임에 컨트롤러와 위빙하도록 하는 방법을 생각해볼 수 있습니다. 실제 보안 프레임워크들에서도 AOP방법론에 따라 위빙을
	 		위한 어노테이션을 활용하고 있기는 합니다.
	 		
	 		그렇지만 첫 번째/두 번째 모두 우리가 처리하고 싶은 기능을 구현하는게 제약이 따릅니다.
	 		우리가 지금 처리하고 싶은 기능은 특정 URl에 대한 접근 내역을 기록하거나 특정 경로에 대한 접근 제어를 하는 등 웹이라는 공간과 환경에 제한된
	 		공통 기능을 처리하고 싶은 것입니다. AOP는 너무 범용적이 방법이라 할 수 있고, Filter 방식은 제약이 많습니다. (사용할 자원의 환경이 다름)
	 		이러한 경우에 사용하기 위한 전략으로 스프링은 HandlerInterceptor라는 추상화를 제공하고 있으며, 이를 사용하면 Spring MVC에 맞게
	 		공통 기능을 다수의 URL에 적용할 수 있게 됩니다.
	 		...
	 		preHandle() 메소드는 컨트롤러 객체를 실행하기 전에 필요한 기능을 구현할 때 사용되며, Handler 파라미터는 웹 요청을 처리할 컨트롤러
	 		객체입니다. 이 메소드를 사용하면 컨트롤러를 실행하기 전에 컨트롤러에서 필요로 하는 정보를 생성하거나 접근 권한이 없는 경우 리턴값으로 false를
	 		반환하여 컨트롤러가 실행되지 않도록 하는 작업이 가능합니다.
	 		
	 		postHandler() 메소드는 컨트롤러가 정상적으로 실행된 이후에 추가 기능을 구현할 때 사용되는데, 만약 컨트롤러에서 예외가 발생했다면
	 		postHandler() 메소드는 실행되지 않습니다.
	 		
	 		afterCompletion() 메소드는 클라이언트에 뷰를 전송한 뒤에 실행되며, 만약 컨트롤러를 실해하는 관점에서 예외가 발생했다면, 해당 메소드의
	 		네 번째 파라미터로 전달된다.
	 		
	 		예외가 발생하지 않을 경우는 null값이 들어올 것입니다. 따라서 컨트롤러 실행 이후 예기치 않은 예외에 대해 로깅을 한다거나 실행 시간을 기록하는 등의
