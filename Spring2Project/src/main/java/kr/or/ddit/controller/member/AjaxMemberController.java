...
public class AjaxMemberController {
	...
	public ResponseEntity<String> ajaxRegister04(String userId){
		...
		// 타입으로 데이터를 받는다.
		// 물론 기본 타입의 해당하는 String으로도 받을 수 있지만, 보내는 쪽에서 JSON 객체의 형태를 태워 보내는게 아니라 값만 보내는 형식이기
	...
	public ResponseEntity<String> register08(@RequestBody Member member){
		...
		if(address != null) {
			log.info("address.postCoe : " + address.getPostCode());
	...
	// 9) 중첩된 객체 타입의 JSON 요청 데이터를 @RequestBody 어노테이션을 지정하여 중첩된 자바빈즈 매개변수로 처리한다.(리스트)
