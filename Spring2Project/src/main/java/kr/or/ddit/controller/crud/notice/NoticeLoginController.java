...
public class NoticeLoginController {
	...
	public String signup(
		...
		if(errors.size() > 0) {	// 에러 발생
			...
			if(result.equals(ServiceResult.OK)) {
				ra.addFlashAttribute("message", "회원가입을 완료 하였습니다.");
	...
	public ResponseEntity<String> idForgetProcess(@RequestBody NoticeMemberVO member) {
		log.info("회원 이메일 : " + member.getMemEmail());
		log.info("회원 이름 : " + member.getMemName());
		
		String result = noticeService.findId(member);
	...
	public ResponseEntity<String> pwForgetProcess(@RequestBody NoticeMemberVO member) {
		log.info("아이디 : " + member.getMemId());
		log.info("이메일 : " + member.getMemEmail());
		log.info("이름 : " + member.getMemName());
		
		String result = noticeService.findPw(member);
		return new ResponseEntity<String>(result, HttpStatus.OK);
	}
