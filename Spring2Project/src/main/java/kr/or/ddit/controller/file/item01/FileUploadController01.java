...
public class FileUploadController01 {
	 ...
	 * 		1. 파일업로드 설명
	 * 
	 * 			- 서블릿 3.0에서 지원하는 파일 업로드 기능과 스프링 웹에서 제공하는 컴포넌트를 이용하여 파일을 업로드한다.
	 * 
	 * 			# 파일 업로드 설정
	 * 			1) 서블릿 3.0 이상 지원
	 ...
	 * 				- StandardServletMultipartResolver 사용시 설정,
	 ...
	 * 
	 * 				- CommonsMultipartResolver 사용시 설정, <<< 이녀석이 우리가 사용할 녀석
	 ...
	 * 				- 파일업로드 등록 Mapper 인터페이스 메소드 만들기
	 * 				- 파일업로드 등록 기능 Mapper xml 뭐리 만들기
	...
	// root-context.xml 에서 설정한  uploadPath 라는 이름으로 빈 등록 된 value의 경로 값을 가져온다.
	...
	public String itemRegister(Item item, Model model) throws Exception, Exception {
		...
		itemService.register(item);	// 일반데이터 및 파일데이터 DB 등록
