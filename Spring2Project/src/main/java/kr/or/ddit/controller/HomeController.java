...
public class HomeController {
	 ...
	 * 			viewResolver가 잡아 페이지를 처리하는것처럼 반응해서 404에러가가 발생한다.
	...
	public Member goHome0301() {
		 ...
		 * @ResponseBody를 설정하지 않고 객체를 응답으로 내보낼 시, 데이터로 인식되는게 아닌 페이지로 인식이
		 * 되므로 404 에러가 발생하므로 어노테이션을 꼭 붙여줘야한다.
		 */
	...
	public ResponseEntity<Void> goHome0601(){
		...
		// 확인할 수 있는데, 이때 상태코드 200만 정상 응답으로 나간걸 확인할 수 있다.
	 ...
	 */
	// 객체의 JSON 객체 타입의 데아터와 200 OK 상태코드를 전송한다.
	@ResponseBody
	 ...
	 * 
	 * 		무료/유료 이미지 다운로드 홈페이지를 사용해보면 이미지 미리보기 또는 미리보기 후 다운로드를 할 수 있는 기능이 제공됩니다.
	...
	public ResponseEntity<byte[]> goHome1101(){
		...
		try {
			in = new FileInputStream("D:\\00.JSP_SPRING\\02.SPRING2\\image\\screenshot01.png");
