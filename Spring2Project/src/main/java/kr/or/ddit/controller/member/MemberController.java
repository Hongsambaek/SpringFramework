...
import kr.or.ddit.vo.test.TestMember;
...
public class MemberController {
	...
	// 	받고자 하는 데이터의 형식에 맞는 타입을 내가 직접 설정할 수 있다.
	...
	// 2) 폼 텍스트 플드 요소값을 자바빈즈 매개변수와 기본 데이터 타입인 정수 타입 매개변수로 처리한다.
	...
	// 2) Member 매개변수와 폼 방식 요청 전달받은 값이 날짜 문자열 형식으로 설정 시, Date 타입으로 받을 수 있는가?
	...
	// 기본 타입에 해당하는 Date타입으로 데이터를 받거나 Member 객체 안의 Date타입으로 설정해둔 필드로 받을 때
	// @DateTimeFormat을 이용하여 pattern 속성으로 내가 받고자 하는 데이터 형식을 만들었을때 데이터를 바인딩 할 수 있다.
...
//		log.info("dateOfBirth : " + dateOfBirth);
	...
	public String registerCheckbox02(String[] hobbyArray) {
		...
		if(hobbyArray != null) {
			log.info("hobbyArray is not null");
	...
	// ****
	// 11) 폼 체크박스 요소값을 문자열 요소를 가진 리스트 컬렉션 매개변수로 처리한다.
	...
	public String registerCheckbox03(ArrayList<String> hobbyList) {
		...
		// 스프링에서는 List타입으로 데이터를 받는데에 문제가 있다.(아무런 헬퍼 없이 기본 데이터 타입으로 받을때 데이터 바인딩이 안됨)
		...
		if(hobbyList != null) {
			log.info("hobbyList is not null");
	...
	// registerAllForm 페이지에서 데이터 입력후 데이터를 전송할 메소드
	...
	@RequestMapping(value="/registeFile07", method = RequestMethod.POST)
	...
	// 8) 파일업로드 폼 파일 요소값과 텍스트 필드 요소값을 MultipartiFile 타입의 배열 매개변수로 처리한다.
