...
public class TilesSettingsController {
	 ...
	 * 		변화하는 부분에 대해서만 동적으로 변환해 페이지를 관리 할 수 있어야할 것이다.
	 * 		이렇게, header/footer/menu 등 공통적인 소소를 분리하여 한 화면에서 동적으로 레이아웃을 한곳에 배치 하여 설정하고 관리할 수 
	 * 		있도록 도와주는 페이지 모듈화를 돕는 프레임워크이다.
	 * 
	 * 		- 아래 jsp들을 이용하여 페이지 모듈화를 진행
	 * 		배경지가 될 template.jsp
	 * 		헤더 영역을 담당하는 header.jsp
	 ...
	 * 					그래서 되는 마이너 하위버전으로 수동 설치를 진행함(되도록이면 정상작동하는 플러그인을 사용하면 좋을텐데...)
	 * 				
	 * 				# Mybatipse plugins 수동 설치 방법
	 * 				- 이클립스 실행 폴더 내, feature 폴더에 jar파일 추가
	 * 				- 이클립스 실행 폴더 내, plugins 폴더에 jar파일 추가
	 * 
	 * 
	 * 			2. log4jdbc.log4j2 설정
	 * 				- log4jdbc.log4j2 라이브러리 설정
	 * 				- log4jdbc.log4j2.properties 설정
	 * 				- dataSource property 수정
	 * 
	 * 
	 * 
	 * 			3. 공지사항 게시판 실습
	 ...
	 * 
	 * 				3-2) 게시판 작성
	 * 					- 게시판 목록, 상세보기 컨트롤러 만들기 (notice/NoticeRetreiverController)
