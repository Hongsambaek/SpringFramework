...
// @Aspect는 어노테이션을 붙여 해당 클래스가 Aspect를 나타내는 클래스라는걸 명시
// AOPController 클래스는 빈 등록 시, aOPController로 할지 aoPController로 할지가 명확하지 않을 수 있으므로
...
public class AOPController {
	...
	/*
	  	[ 14장. AOP ]
	  		...
	  		필거는 본인이 만들어 낸 기능을 각 서비스 로직에 하나하나씩 약 2만개쯤 작성 하던중, 의문을 갖기 시작합니다...
	  			...
	  			"서비스 로직에서 제일 중요한 로직은 본래의 기능인 비즈니스 로직인데 지금 내가 작성하는 로직은 옵션(부가 기능)인데...
	  		...
	  		이러한 인프라 로직은 아래처럼 하나의 관심사를 가질 수 있는데 이런 관심사들의 중복이 횡단으로 나타나는데, 이것을 가르켜
	  		'횡단 관심사(Cross-Cutting-Concern)' 이라고 합니다.
	  
	  		┌───────────────────────────────────────────────────────────────────────────┐
	  		│		[처리속도측정]		[처리속도측정]		[처리속도측정]		[처리속도측정]			│
	  		└───────────────────────────────────────────────────────────────────────────┘
	  				[비즈니스로직]		[비즈니스로직]		[비즈니스로직]		[비즈니스로직]
	  				[처리내용로직]		[처리내용로직]		[처리내용로직]		[처리내용로직]
	  		─────────────────────────────────────────────────────────────────────────────
	  		...
	  		- @Component
	  			> @Aspect와 짝궁, @Component는 component-scan시 '저 여기있어요! 여기 봐주세요~' 라는 의미
	  		- JoinPoint
	  			> 어드바이스가 적용될 수 있는 위치
	  		- Advice
	  			> 어떤 부가기능(메소드 실행 시작 시간 출력)을 언제 (삽겹살을 굽기 전(Before)에) 사용할지 정의
	  				> Advice(부가기능)는 타겟을 감싸서 위장된 프록시가 실행되기 위한 시점에 따라 옵션이 나뉘어진다.
	  					...
	  					- After Throwing : 조인 포인트에서 예외 발생 시 실행. 예외가 발생한다면 실행되지 않는다.
	  					- Around : 조인 포인트 전후에 실행(삼겹살을 굽기 전과 먹은 후에 실행)
	 			...
	 			- 관점 지향 프로그래밍을 의미하는 약자
	  			...
	  			1-1) 관점 지향 프로그래밍
	 				...
	 				간단하게 설명하면 관점 지향 프로그래밍은 횡단 관심사의 분리를 실현하는 방법입니다.
	  					...
	  					- 횡단 관심사 분리(Separation Of Cross-Cutting Concern)
	 					: 애츨리케이션을 개발할 때 횡단 관심사에 해당하는 부분을 분리해서 한 곳으로 모으는 것을 의미합니다.
	 
	  		
	  			1-2) AOP 개발 순서
	  				(1) 핵심 비즈니스 로직에만 근거해서 코드를 작성합니다.
	  				(2) 주변 로직에 해당하는 관심사들을 분리해서 따로 작성합니다.
	  			...
	  			## 스프링 AOP
	  				스프링 AOP는 동작 시점이 여러가지가 존재합니다. (컴파일 시점, 클래스 로딩 시점, 런타임 시점 등등...)
	  				그 중, 런타임 시점에 프록시 객체를 생성 후 기능을 삽입합니다.
	  					> 스프링 AOP의 프록시는 메소드의 오버라이딩(인터페이스 기반의 참조) 개념으로 동작 하므로, 메소드 실행 시점 시 동작
	  		...
	  		- excution 지시자에 대해서 알아봅시다.
	  			...
	  			- Advice가 실행될 지점을 표현하는 방식
	  				...
	  				2-1) execution 지시자의 표현 방식
	  					...
	  					- 포인트 컷 요소
	  					예) execution(Board kr.or.ddit.service.IBoardService.BoardServiceImpl*.read*(..))
	  				...
	  				2-2) 포인크컷 표현식에 사용되는 와일드카드
	  				
		  				────────────────────────────────────────────────────────────────────────────────────────────────────────────────
		  					와일드 카드		|				설명
		  				────────────────────────────────────────────────────────────────────────────────────────────────────────────────
		  					*			| 임의의 패키지 1개 계층을 의미하거나 임의이 인수 1개를 의미한다.
		  					..			| 임의의 패키지 0개 이상 계층을 의미하거나 임의이 인수 0개를 의미한다.
	...
	/*
	 * 	  3. Before 어드바이스
	 *		- 조인포인트 전에 실행된다. 예외가 발생하는 경우만 제외하고 항상 실행된다.  
	 */
	 ...
	 * 			책임이 around 어드바이스로 전가되고 비즈니스 메소드에 대한 정보를 around 어드바이스 메소드가 가지고 있어야 가오
	 * 			해당 정보를 스프링 컨테이너가 around 어드바이스 메소드로 넘겨주면 ProceedingJoinPoint 객체로 받아서
